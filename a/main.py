...
class Win(object):
    def __init__(self):
        ...
        # 绑定消息列表
        self.mes_list = Text(self.frame_1, width=52,
                             height=20, bd=0, padx=5, pady=5,state=DISABLED)
        self.mes_list.grid(row=0, column=0)
        # 绑定输入消息内容
        self.mes_txt = Text(self.frame_2)
        self.mes_txt.grid()
        # 绑定好友列表
        self.friend_list = Text(self.frame_3)
        self.friend_list.grid()
        # 板锭发送按钮
        self.mes_send = Button(self.frame_11, text=('发送'),
                               command=self.fun_send)
        self.mes_send.grid()
